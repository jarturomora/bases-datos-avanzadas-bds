...
run_js () {
  local uri="$1" expr="$2"
  for i in {1..25}; do
    echo "Ejecutando: ${expr} (intento ${i}/25) en ${uri}"
  ...
  echo "ERROR: no se pudo ejecutar: ${expr} después de 25 intentos" >&2
  exit 1
}
...
# 4) Esperar a mongos y configurar cluster + datos
wait_ping "mongodb://mongos:27017" "mongos"
run_js "mongodb://mongos:27017" "configureClusterAndData()"

echo "Inicialización completada (distributed queries demo)."
